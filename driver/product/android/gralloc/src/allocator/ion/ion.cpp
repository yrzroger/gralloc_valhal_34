...
/* Used in allocator_allocate to initialize a private_handle_t after its creation. */
static int init_handle_non_protected(private_handle_t *handle, const buffer_descriptor_t *descriptor, ion_device *dev)
{
	int ret = allocator_map(handle);
	if (ret != 0)
	{
...
int allocator_allocate(const buffer_descriptor_t *descriptor, private_handle_t **out_handle)
{
	...
	android::base::unique_fd shared_fd{
		dev->alloc_from_ion_heap(usage, descriptor->size, heap_type, ion_flags)};
	if (shared_fd < 0)
	{
	...
	if (!(usage & GRALLOC_USAGE_PROTECTED))
	{
		ret = init_handle_non_protected(handle, descriptor, dev);
		if (ret != 0)
		{
